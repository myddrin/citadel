        ...
        for (c = 0; c < characters.size(); c++)
		{
			...
			while (-1 == choice)
			{
				...
				if (choice) // != 0 take card
				{
					...
					// for each to keep
					while (kept < (int)current->nToKeep())
					{
				...
				kept = 0;
				while (kept < (int)current->nToBuild())
				{
					choice = -1;
...
int Game::power_kill(Citadel::Player *current)
{
    ...
    if (choice > (int)character->level
        && choice < (int)characters.size())
    {
    ...
    while (choice < 0)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (choice < (int)players[c]->built().size())
                {
